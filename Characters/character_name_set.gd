...
func get_random_male_name() -> String:
	...
	var i: int = randi_range(0, male_names.size() - 1)
	assert(i >= 0 and i < male_names.size())
	
	var new_first_name: String = male_names[i]
	
	i = randi_range(0, last_names.size() - 1)
	assert(i >= 0 and i < last_names.size())
	
	var new_last_name: String = last_names[i]
...
func get_random_female_name() -> String:
	...
	var i: int = randi_range(0, female_names.size() - 1)
	assert(i >= 0 and i < female_names.size())
	
	var new_first_name: String = female_names[i]
